...
const ADD_FILE = 'dbe-12-picks.ics';
const CANCEL_FILE = 'dbe-12-remove.ics';
const EXPORTED_KEY = 'dbe12.cal.exported.v1';

const dayDate = new Map(DAYS.map((d) => [d.id, d.date]));

/**
 * The festival runs 12–15 August 2026, firmly inside Eastern European Summer
...
function toUtc(dayId: NightId, hhmm: string): Date | null {
...
/** Date -> iCalendar UTC stamp, e.g. 20260813T183000Z. */
...
function uidOf(slot: SetSlot): string {
  return `${slot.dayId}-${slug(slot.band)}@dbe2026`;
}
...
function vevent(
  ...
  // While the running order is provisional, say so inside the calendar entry —
  // the reminder is only ever as good as the time it was built from.
  const provisional = slot.tba ? ' (provisional time)' : '';
  ...
  const lines = [
    ...
    `DTEND:${stampOf(end)}`,
    `SUMMARY:${esc(`${slot.band}${provisional}`)}`,
    `LOCATION:${esc(FESTIVAL.location)}`,
  ];
...
function wrapCalendar(method: 'PUBLISH' | 'CANCEL', events: string[]): string {
  return [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//dbe2026//Night Planner//EN',
    'CALSCALE:GREGORIAN',
    `METHOD:${method}`,
    `X-WR-CALNAME:${esc(`${FESTIVAL.shortName} 12 — my nights`)}`,
    ...events,
    'END:VCALENDAR',
    '',
  ].join('\r\n');
...
export async function exportCalendar(): Promise<CalendarResult> {
  ...
  const result = await deliver(ics, ADD_FILE, `${FESTIVAL.shortName} 12 — my nights`);
...
export async function clearCalendar(): Promise<CalendarResult> {
  ...
  if (!ics) return { outcome: 'empty' };
  const result = await deliver(ics, CANCEL_FILE, `${FESTIVAL.shortName} 12 — remove`);
  forgetExported();
  return result;
}

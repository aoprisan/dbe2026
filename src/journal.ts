 ...
 * 🕯 candles, mark the ones you didn't make it to, and keep a one-line memory.
 * Everything stays on-device, and it all feeds the shareable "Rewind" recap.
 */

const RATING_KEY = 'dbe12.journal.ratings.v1';
const SKIP_KEY = 'dbe12.journal.skips.v1';
const NOTE_KEY = 'dbe12.journal.notes.v1';
...
const ratings = new Map<string, number>(
  Object.entries(loadRecord(RATING_KEY)).filter(
    (e): e is [string, number] =>
      typeof e[1] === 'number' && e[1] >= 1 && e[1] <= MAX_CANDLES,
  ),
);
...
export function setRating(id: string, candles: number): void {
  if (candles <= 0) ratings.delete(id);
  else ratings.set(id, Math.min(MAX_CANDLES, Math.round(candles)));
...
function buildDialog(): HTMLDialogElement {
  ...
  d.className = 'sheet journal';
  d.setAttribute('aria-label', 'Festival journal');

  const card = el('div', 'sheet-card');

  const head = el('div', 'sheet-head');
  head.appendChild(el('h2', 'sheet-title', '🕯 Festival journal'));
...
function repaint(): void {
  ...
  if (ended.length === 0) {
    const empty = el('p', 'sheet-empty');
    empty.textContent =
      selection.size() === 0
        ? 'Pick some acts first — once their sets have played, you can rate them here and build your shareable Rewind.'
...
function renderEntry(slot: SetSlot): HTMLElement {
  ...
  row.style.setProperty('--c', slot.night.color);

  const top = el('div', 'journal-entry-top');
  top.appendChild(
    el('span', 'row-time', `${slot.tba ? '~' : ''}${slot.startLabel}–${slot.endLabel}`),
  );
  const band = el('span', 'row-band', slot.band);
  band.style.setProperty('--c', slot.night.color);
  top.appendChild(band);
  row.appendChild(top);

  const controls = el('div', 'journal-controls');
  controls.appendChild(renderCandles(slot.id));
  ...
  if (!isSkipped(slot.id)) {
    ...
    noteInput.placeholder = 'One-line memory… (the drop, the smoke, the silence?)';
...
function renderCandles(id: string): HTMLElement {
  const wrap = el('div', 'journal-candles');
  wrap.setAttribute('role', 'radiogroup');
  wrap.setAttribute('aria-label', 'Rate this set, 1 to 5 candles');
  const skipped = isSkipped(id);
  const current = rating(id);
  for (let n = 1; n <= MAX_CANDLES; n++) {
    const btn = el('button', 'journal-candle', CANDLE);
    btn.type = 'button';
    btn.disabled = skipped;
    if (n <= current) btn.classList.add('is-on');
    btn.setAttribute('aria-label', `${n} candle${n > 1 ? 's' : ''}`);

...
/**
 * Crew mode: your friends' nights, layered over yours. A friend is added by
 * pasting the `#p=…` picks link they shared — the same compact bitmask token
 * the app already generates — so the whole feature stays client-side with no
 * accounts and no backend. From the overlays we derive who's at each set, which
 * sets you'll be together for, and the "meet-up windows" when the entire crew
...
const CREW_KEY = 'dbe12.crew.v1';

/** Distinct, readable inks kept clear of the four night hues. */
const PALETTE = ['#7fb3ff', '#e5709a', '#8fd694', '#e0c76a', '#b98cff', '#5fc9c1', '#e2915a', '#9fb0c9'];
...
/** Sets you picked that at least one friend also picked, in running order. */
...
export interface DayMeetups {
  dayId: string;
  /** People (you + friends) with at least one pick that night. */
  participants: number;
  windows: MeetWindow[];
}

/**
 * Gaps where *everyone* who is there that night (you plus every friend with a
...
function repaint(): void {
  ...
  body.appendChild(
    el(
      'p',
      'sheet-intro',
      'Ask a friend for their picks link (Options → 🔗 Share picks link), paste it here, and their nights overlay yours: badges on the running order, the sets you’ll be together for, and the windows when the whole crew is free to meet.',
    ),
  );
...
function renderAddForm(): HTMLElement {
  ...
  const name = el('input', 'field crew-input-name') as HTMLInputElement;
  name.placeholder = 'Friend’s name';
  name.setAttribute('aria-label', 'Friend’s name');
  name.maxLength = 24;
  name.required = true;

  const link = el('input', 'field crew-input-link') as HTMLInputElement;
  ...
  const add = el('button', 'pill-btn', 'Add to crew') as HTMLButtonElement;
  add.type = 'submit';
  ...
  for (const day of DAYS) {
    ...
    if (together.length > 0) {
      ...
      for (const t of together) {
        ...
        band.style.setProperty('--c', t.slot.night.color);
        li.appendChild(band);
        li.appendChild(
          el(
            'span',
            'crew-together-meta',
            `${t.slot.tba ? '~' : ''}${t.slot.startLabel} · with ${t.with.map((f) => f.name).join(', ')}`,
          ),
        );
        ul.appendChild(li);
      }
    ...
    if (meet && meet.windows.length > 0) {
      section.appendChild(
        el('p', 'crew-meet-head', `Meet-up windows (all ${meet.participants} of you free):`),
      );

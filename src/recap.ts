...
import { CANDLE, note, rating, seenSlots } from './journal';
import {
  COLORS,
  SHARE_URL,
  drawPill,
...
/**
 * "My Dark Bombastic Rewind" — a Wrapped-style recap of the festival you
 * actually had, rendered to a shareable PNG: sets seen, hours in front of the
 * stage, your top-rated podium and the sounds that defined the four nights.
 * Built entirely from the on-device journal; works mid-festival as a running
 * tally.
 */

const FILE_NAME = 'dbe-12-rewind.png';
...
export async function shareRecap(): Promise<ShareResult> {
  ...
  return sharePngBlob(blob, FILE_NAME, {
    title: `My ${FESTIVAL.shortName} 12 Rewind`,
...
function renderRecapPng(seen: SetSlot[]): Promise<Blob> {
  ...
  for (const s of seen) perNight[s.dayId] += 1;
  ...
  const H = HEADER_H + TILES_H + podiumH + QUOTE_H + GENRE_H + NIGHT_H + FOOTER_H;
  ...
  nameGrad.addColorStop(0, COLORS.ceremony);
  nameGrad.addColorStop(0.5, COLORS.accent);
  nameGrad.addColorStop(1, COLORS.sat);
  ctx.fillStyle = nameGrad;
  ctx.font = '800 28px "Segoe UI", system-ui, sans-serif';
  ctx.fillText('MY DARK BOMBASTIC REWIND', PAD, 56);
  ...
  pillX = drawPill(
    ...
    pillX,
    pillY,
    `${byDay.size} night${byDay.size === 1 ? '' : 's'} in the dark`,
    COLORS.text,
    COLORS.line,
    COLORS.panel,
  );
  ...
  drawTile(ctx, PAD + tileW + 16, y, tileW, 84, fmtDuration(musicMin), 'in front of the stage');
  y += TILES_H;
  ...
  if (podium.length > 0) {
    ...
    podium.forEach((slot, i) => {
      ...
      ctx.fillStyle = COLORS[slot.dayId];
      ctx.fill();
      ...
      ctx.textAlign = 'right';
      ctx.font = '600 14px "Segoe UI", system-ui, sans-serif';
      ctx.fillText(CANDLE.repeat(rating(slot.id)), W - PAD - 12, rowY + 26);
      ctx.textAlign = 'left';
    });
  ...
  if (topGenres.length > 0) {
    ctx.fillStyle = COLORS.text;
    ctx.font = '800 18px "Segoe UI", system-ui, sans-serif';
    ctx.fillText('My nights sounded like', PAD, y + 20);
    y += 34;
  ...
  let sx = PAD;
  const sy = y + 30;
  for (const id of ['ceremony', 'thu', 'fri', 'sat'] as const) {
    if (perNight[id] === 0) continue;
    ctx.beginPath();
    ...
    const text = `${NIGHTS[id].short} ${perNight[id]}`;
  ...
  ctx.fillText('My festival, from the unofficial night-planner journal', W / 2, H - 36);

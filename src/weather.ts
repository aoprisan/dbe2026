...
// Cache the last good forecast so the panel still shows something on site with
// patchy signal (this is an offline-first PWA). Namespaced with a schema
// version: bump the suffix whenever the cached shape changes so an older
// build's entry is ignored rather than rendered without the new fields.
const CACHE_KEY = 'dbe12:weather:v1';
const CACHE_TTL_MS = 60 * 60 * 1000; // an hour is plenty for a daily forecast

// The first set is at 18:30 and the curfew stops the music before midnight, so the
...
// The app is meant to stay open all evening on site, so keep the forecast fresh
// without a hard reload: re-check on a timer and whenever the tab regains focus.
// Both paths only hit the network when the cache is actually stale, so a
// backgrounded tab or rapid refocusing won't hammer the API.
 ...
 * every live view. A no-op while the cache is still fresh or a refresh is
 * already running, so it's cheap to call speculatively.
 */
...
async function doRefresh(): Promise<void> {
  try {
    ...
    hourIndex = new Map(hours.map((h) => [h.time, h]));
    notifyForecast(); // repaints the running order's per-set icons
...
function buildDialog(): HTMLDialogElement {
  ...
  sub.textContent = `${FESTIVAL.location} · tap a night for the hourly forecast`;
...
async function fetchForecast(): Promise<{ days: DailyForecast[]; hours: HourForecast[] }> {
  ...
  const res = await fetchAvailableForecast(params);
...
/**
 * Open-Meteo's forecast window advances one day at a time. Near the edge of
 * that window our desired extra after-midnight day may not be available yet,
 * even though earlier festival days are. Shorten only an out-of-range end date
 * until the API accepts the largest currently available slice.
 */
async function fetchAvailableForecast(params: URLSearchParams): Promise<Response> {
  const earliestEnd = firstDate();
  let endDate = params.get('end_date') ?? earliestEnd;

  while (endDate >= earliestEnd) {
    ...
    let reason = '';
    try {
      const problem = (await res.json()) as { reason?: unknown };
      if (typeof problem.reason === 'string') reason = problem.reason;
    } catch {
      // A non-JSON error is not a date-window response, so do not retry it.
    }

    const endDateOutOfRange =
      res.status === 400 &&
      reason.includes("Parameter 'end_date'") &&
      reason.includes('out of allowed range');
    if (!endDateOutOfRange || endDate === earliestEnd) {
  ...
  throw new Error('No festival dates are currently inside the forecast window');
}
...
function renderHours(container: HTMLElement, date: string): void {
  ...
  if (available.length === 0) {
    container.textContent = 'Hourly forecast not available yet.';
    container.className = 'weather-hours weather-hours-empty';
    return;
  }

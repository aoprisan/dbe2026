...
const ENABLED_KEY = 'dbe12.notify.enabled.v1';
const LEAD_KEY = 'dbe12.notify.lead.v1';
const FIRED_KEY = 'dbe12.notify.fired.v1';
...
async function show(slot: SetSlot, lead: number): Promise<void> {
  ...
  const body = `${slot.startLabel}${slot.tba ? ' (provisional)' : ''} · ${slot.night.name}`;
